 ...
 * Lab 3, Task 1, Part 1
 * 
 * I did not make this file - provided by staff.
 * I implemented the on board LEDs for use in
 * the LED_Init function.
 * I followed the directions for ADCReadPot_Init.
 * I implemented TimerADCTriger_Init to trigger the
 * ADC. I initialized Timer 0 in this function and had
 * it count down from 5 seconds.
 */
...
void TimerADCTriger_Init(void) {
  ...
  GPTMCFG_0 = 0x0;         // set configuration
  GPTMTAMR_0 = 0x2;        // periodic count down mode
  
  GPTMTAILR_0 = 60000000;  // 5 second value for timer
  GPTMCTL_0 |= (1 << 5);   //
  GPTMADCEV_0 = 0x1;       // trigger enable
  GPTMCTL_0 |= 0x1;        // enable timer 0a
}
